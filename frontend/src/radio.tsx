...
import { queryProducts, resetPagination } from './query';
...
export const TagRadios = (props: Props) => {
  ...
  const setProductGender = (event: any) => {
    if (event.target.value === props.gender) {
      ...
      resetPagination();
      void queryProducts(props, event.target.value, props.category)
    }
  }
  const setProductCategory = (event: any) => {
    if (event.target.value === props.category) {
      ...
      resetPagination();
      void queryProducts(props, props.gender, event.target.value)
    }
  }
  return (
    <FormControl>
